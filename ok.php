...
echo color("Maroon","[•]         SCRIP ANAK TANJUNGBALAI           \n");
echo color("navy","[•] 			CROWLEY \n");
echo color("yellow","# # # # # # # # # # # # # # # # # # # # # # # \n");
function change(){
        $nama = nama();
        ...
        $no = trim(fgets(STDIN));
        $data = '{"email":"'.$email.'@palui.hmail.us","name":"'.$nama.'","phone":"+'.$no.'","signed_up_country":"ID"}';
        ...
        echo "\n".color("nevy","?] Mau Nembak?: y/n ");
        $pilihan = trim(fgets(STDIN));
        if($pilihan == "y" || $pilihan == "Y"){
        echo color("red","===========(REDEEM VOUCHER)===========");
        echo "\n".color("blue","!] Claim voc GORIDE 8K");
        ...
        $message = fetch_value($code1,'"message":"','"');
        if(strpos($code1, 'vocher berhasil, lanjut menembak')){
        echo "\n".color("green","+] Message: ".$message);
        goto gofood;
        }else{
        echo "\n".color("red","-] Message: ".$message);
        ...
        if(strpos($claim, 'vocher berhasil, lanjut menembak')){
        echo "\n".color("green","+] Message: ".$message);
        goto gofood;
        }else{
        echo "\n".color("red","-] Message: ".$message);
